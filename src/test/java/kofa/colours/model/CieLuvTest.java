...
class CieLuvTest {
    ...
    void toXyz_white() {
        assertIsCloseTo(
                new CieLuv(100, 0, 0).toXyz().usingD65(),
                D65_WHITE_XYZ,
                PRECISE
        );
    }
    ...
    void toXyz_black() {
        assertIsCloseTo(
                new CieLuv(0, 0, 0).toXyz().usingD65(),
                new Xyz(0, 0, 0),
                PRECISE
        );
        assertThat(new CieLuv(0, 100, -100).toXyz().usingD65().Y()).isEqualTo(0);
    }

    @Test
    void toXyz_L_below_kappa() {
        assertIsCloseTo(
                new CieLuv(7.99999, 1, 1).toXyz().usingD65(),
    ...
    void fromXyz_white() {
        assertIsCloseTo(
                CieLuv.from(D65_WHITE_XYZ).usingD65(),
                new CieLuv(100, 0, 0),
                PRECISE
        );
    }
    ...
    void fromXyz_black() {
        assertIsCloseTo(
                CieLuv.from(new Xyz(100, 0, -100)).usingD65(),
                new CieLuv(0, 0, 0),
                PRECISE
        );
    }
    ...
    void fromXyz_yr_below_epsilon() {
        assertIsCloseTo(
                CieLuv.from(new Xyz(0.0086492080771262, 0.00885644060847103, 0.00842659724738038)).usingD65(),
